...
export default function SignUp({setLogin}) {
    const [errors, setErrors] = useState({email: false, password: false, repeat: false})
    const [form, setForm] = useState({email: '', password: '', repeat: ''})

    const formFilled = (e) => {
        e.preventDefault()
        setErrors({email: form.email === '', password: form.password === '', repeat: form.repeat === ''})
    }
    return(
        <div className="form-box">
            <Form onSubmit={formFilled}>
                ...
                <Form.Group className="form-group" as={Row}>
                    <Col xs={'auto'} className={`p-0 ${!errors.repeat && 'w-100'}`}>
